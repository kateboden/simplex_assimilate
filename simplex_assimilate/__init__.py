...
def likelihood(alpha: NDArray[np.float64], pre_x: NDArray[np.uint32]) -> np.float64:
    ...
    j = len(pre_x)
    agrees = np.all((alpha[:j]>0) == (pre_x>0)) and (alpha[j:].sum()>0)==(pre_x.sum()<ONE)  # zeros and non-zeros agree
    alpha = np.append(alpha[:j], alpha[j:].sum())  # collapse x_(>=j) into a single component
    out = stats.dirichlet.pdf(pre_x[pre_x>0]/ONE, alpha[alpha>0]) if agrees else 0  # return the pdf if pre_x fits alpha, else return 0
    # check outputs
...
def cdf(x_j, prior: dirichlet.MixedDirichlet, pre_samples: NDArray[np.uint32]) -> NDArray[np.float64]:
    ...
    betas = stats.beta(middle_alphas[:, j], middle_alphas[:, j + 1:].sum(axis=1))
    middle_cdfs = betas.cdf(x_j / upper).reshape((N, -1))
    # BUILD THE CDF from the LOWER, MIDDLE, and UPPER CLASSES
    lower_mass = posterior_pi[:, lower_classes].sum(axis=1)
    posterior_middle_weights = posterior_pi[:, middle_classes]
    assert middle_cdfs.shape == (N, middle_classes.sum()) == posterior_middle_weights.shape
    out = np.where(x_j==0, lower_mass, np.where(x_j>=upper, 1., lower_mass + (posterior_middle_weights * middle_cdfs).sum(axis=1)))
    assert (out < 1+1e-10).all(), 'cdf must be less than 1'
    out = np.minimum(out, 1)  # clip to 1 (numerical error in posterior_weights can cause it to be slightly greater than 1
    # check output
...
def inv_cdf(uniforms: NDArray[np.float64], prior: dirichlet.MixedDirichlet,
    ...
    X[middle_mask] = vector_binary_search(lambda x: cdf(x, prior,  pre_samples), uniforms[middle_mask])
    # check output
...
def uniformize(samples: NDArray[np.uint32], prior: dirichlet.MixedDirichlet) -> NDArray[np.float64]:
    ...
    for j in range(J):
        ...
        for pc in np.unique(sample_pre_classes, axis=0):
            ...
            u[low_samples] = stats.uniform(0, cdf(0, prior,  pre_samples[pc_idx][low_samples])).rvs()
            u[upper_samples] = stats.uniform(
                cdf(upper[upper_samples] - DELTA, prior,  pre_samples[pc_idx][upper_samples]),
                1 - cdf(upper[upper_samples] - DELTA, prior,  pre_samples[pc_idx][upper_samples])).rvs()
            u[middle_samples] = cdf(x_j[pc_idx][middle_samples], prior,  pre_samples[pc_idx][middle_samples])
            U[pc_idx, j] = u
...
def vector_binary_search(f, Y):
    ...
    X = three[np.arange(len(X)), np.argmin(np.abs(np.column_stack(tuple(f(COL) for COL in three.T)) - Y[:, None]), axis=1)]
